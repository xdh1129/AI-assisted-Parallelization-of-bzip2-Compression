...
\begin{itemize}
  \item \textbf{Workload:} The primary workload for all benchmark sweeps is a $\sim$1.08GB file (1,082,774,528 bytes) produced by concatenating the Canterbury Corpus \texttt{large} reference tarball (11,162,624 bytes) 97 times, providing a realistic and compressible dataset.
...
We swept thread counts $\{1, 2, 4, 8, 16, 32\}$ (block size 900KB, compression level 9, 3 repeats per configuration) against the 1.08GB input described in Section~\ref{subsec:dataset} (the Canterbury Corpus \texttt{large} tarball concatenated 97 times, 1,082,774,528 bytes). Each AI-assisted stage was benchmarked in an isolated git worktree, and lbzip2 was benchmarked as an external reference using the same input and thread sweep.
